...
async function loadKeys() {
    const json = await Api.call(appBase() + '/api/business/api-keys.php?business_id=' + businessId);
...
document.getElementById('key-form').addEventListener('submit', async (e) => {
    e.preventDefault();
    const json = await Api.call(appBase() + '/api/business/api-keys.php', { method: 'POST', body: { business_id: businessId, name: document.getElementById('k-name').value } });
    if (json.success) {
...
async function revokeKey(id) {
    ...
    const json = await Api.call(appBase() + '/api/business/api-keys.php', { method: 'DELETE', body: { business_id: businessId, id } });
    if (json.success) { Toast.success('Key revoked.'); loadKeys(); } else { Toast.error(json.message); }
}

async function loadWebhooks() {
    const json = await Api.call(appBase() + '/api/business/webhooks.php?business_id=' + businessId);
    if (!json.success) { Toast.error(json.message); return; }
...
document.getElementById('webhook-form').addEventListener('submit', async (e) => {
    ...
    const json = await Api.call(appBase() + '/api/business/webhooks.php', { method: 'POST', body: { business_id: businessId, target_url: document.getElementById('w-url').value, events } });
    if (json.success) { Toast.success('Webhook created.'); e.target.reset(); loadWebhooks(); } else { Toast.error(json.message); }
});

async function deleteWebhook(id) {
    if (!confirm('Delete this webhook?')) return;
    const json = await Api.call(appBase() + '/api/business/webhooks.php', { method: 'DELETE', body: { business_id: businessId, id } });
