        ...
        try {
            ...
            $stmt = $pdo->prepare(
                "UPDATE users SET name = ?, email = ?, password_hash = ?, email_verified_at = NOW()
                 WHERE email = 'admin@bharatseo.example'"
            );
...
/**
 * Detects whether this database already contains BharatSEO tables, so the
...
function render_locked_page(): void
{
    ...
    echo '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Already Installed</title>' .
        ...
        '<p>BharatSEO has already been installed on this server.</p>' .
...
<title>Install — BharatSEO</title>
<?= install_styles() ?>
</head>
<body>
<div class="wrap">
    <div class="brand">✨ BharatSEO — Installer</div>
    <div class="card">
        <div class="steps">
        ...
        <?php if ($step === '1'): ?>
            ...
            <p>Checking that your server meets the requirements to run BharatSEO.</p>
            <?php
        ...
        <?php elseif ($step === '2'): ?>
            ...
            <form method="POST" action="<?= install_url('install.php') ?>">
                ...
                <input type="text" name="db_name" placeholder="e.g. username_bharatseo" required>
        ...
        <?php elseif ($step === '3'): ?>
            ...
            <?php if (!empty($_SESSION['install_reused_existing'])): ?>
                <p class="help" style="background:#eef2ff;border-radius:8px;padding:10px 12px;color:#4338ca;">
                    ℹ️ This database already contained BharatSEO tables (likely from an earlier install attempt).
        ...
        <?php elseif ($step === 'done'): ?>
            ...
            <p>BharatSEO has been installed successfully. Your <code>env.php</code> config file was generated automatically, and your admin account is ready.</p>
